...
```bash
./mxcli -p RestLab.mpr    # Correct - uses local binary
```
...
```bash
./mxcli setup mxbuild -p RestLab.mpr    # Auto-download for project's Mendix version
```

After setup, `mx` is at `~/.mxcli/mxbuild/{version}/modeler/mx`. Usage:

```bash
~/.mxcli/mxbuild/*/modeler/mx check RestLab.mpr   # Validate project
./mxcli docker check -p RestLab.mpr               # Alternative (auto-downloads mxbuild)
```
...
```bash
./mxcli -p RestLab.mpr -c "SHOW MODULES"              # List all modules
./mxcli -p RestLab.mpr -c "SHOW STRUCTURE"             # Project overview
./mxcli -p RestLab.mpr -c "SHOW ENTITIES IN MyModule"  # Entities in a module
./mxcli -p RestLab.mpr -c "DESCRIBE ENTITY MyModule.Customer"  # Entity details
```

### Execute an MDL Script File

```bash
./mxcli exec script.mdl -p RestLab.mpr
```
...
```bash
./mxcli
# Then: CONNECT LOCAL 'RestLab.mpr';
```
...
./mxcli check script.mdl -p RestLab.mpr --references  # With reference validation
```
...
```bash
./mxcli check script.mdl -p RestLab.mpr --references
```
...
```bash
# Lint the project
./mxcli lint -p RestLab.mpr

# With colored output
./mxcli lint -p RestLab.mpr --color

# List available rules
./mxcli lint -p RestLab.mpr --list-rules

# Output as SARIF
./mxcli lint -p RestLab.mpr --format sarif > results.sarif
```
...
./mxcli report -p RestLab.mpr

# JSON report
./mxcli report -p RestLab.mpr --format json
